...
class VCSELGenerator:
    def __init__(self, geometry_arrays: list,
    ...
    def _invert_implantation(self,implants,fit=True):
        if fit:
            ...
            minx,miny,maxx,maxy = self._mesa.bounds
            unexposed_region = ShapelyBox(minx,miny,maxx,maxy).difference(implants)
    ...
    def generate_contact_partitions(self, contact_padding = None,
        ...
        for geometry_array in self.geometry_arrays:
            for element in geometry_array.elements:
                ...
                # Remove implant blockers area from the padded element polygon
    ...
    def plot(self, show_mesa=True,
        ...
        if show_contact_region:
            ...
            self._plot_shape(self._contact_region, ax, color='orange', alpha=0.3, label="Contact Region", **kwargs)
        ...
        if show_contact_partitions:
            ...
            for i, partition in enumerate(self._contacts):
                color = kwargs.get('color', None) or 'green'
                alpha = kwargs.get('alpha', 0.5)
                label = "Contacts" if i == 0 else None
                if isinstance(partition, ShapelyPolygon):
                    self._plot_shape(partition, ax, color=color, alpha=alpha, label=label)
                else:
                    for poly in partition.geoms:
                        self._plot_shape(poly, ax, color=color, alpha=alpha, label=label)
                        label = None
        ...
        if show_implant:
            ...
            if isinstance(self._implants, ShapelyPolygon):
                self._plot_shape(self._implants, ax, color=color, alpha=alpha, label=label)
            elif isinstance(self._implants, ShapelyMultiPolygon):
                for i, poly in enumerate(self._implants.geoms):
                    self._plot_shape(poly, ax, color=color, alpha=alpha, label=label if i == 0 else None)
            else:
        ...
        if show_implant_inv:
            ...
            if isinstance(self._implants_inv, ShapelyPolygon):
                self._plot_shape(self._implants_inv, ax, color=color, alpha=alpha, label=label)
...
if __name__ == "__main__":
    ...
    angled_bowtie_generator = VCSELGenerator([angled_array],lateral_growth,
                                             implant_width=22,
                                             implant_length=2,
                                             contact_padding=1,
